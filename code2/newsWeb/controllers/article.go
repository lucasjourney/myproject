...
func(this*ArticleController)ShowIndex(){
	...
	if typeName == "" {
		...
		_, _ = qs.Limit(pageIndex,pageIndex * (pageNum - 1)).RelatedSel("ArticleType").All(&articles)
	}else {
		_, _ = qs.Limit(pageIndex,pageIndex * (pageNum - 1)).RelatedSel("ArticleType").Filter("ArticleType__TypeName",typeName).All(&articles)
	}


	//查询所有文章类型，并展示
	//var articleTypes []models.ArticleType
	//_, _ = o.QueryTable("ArticleType").All(&articleTypes)
	...
	if len(result) == 0 {
		...
		//conn.Do("expire", "newsWeb", "10")
	} else {
		dec := gob.NewDecoder(bytes.NewReader(result))
		dec.Decode(&articlesTypes)
		beego.Info("从redis中获取的文章类型切片成功")
		beego.Info(articlesTypes)
		this.Data["articleTypes"] = articlesTypes
	}
...
func(this*ArticleController)ShowContent(){
	...
	//获取哈哈新闻的文章
	o.QueryTable("Article").Filter("ArticleType__TypeName", "哈哈新闻").RelatedSel("ArticleType").All(&as)
	this.Data["hahaas"] = as

	//获取文章类型
	var articleType models.ArticleType
	o.QueryTable("ArticleType").Filter("Articles__Id", id).One(&articleType)
